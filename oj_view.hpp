...
public:
	...
	static void RenderLogin(std::string* html){
	...
	//渲染登录返回结果
	static void RenderLoginResult(const std::string& reason, std::string* html){
		ctemplate::TemplateDictionary dict("loginResult");
		dict.SetValue("reason", reason);
		ctemplate::Template* tpl;
		tpl = ctemplate::Template::GetTemplate(
          "./template/loginResult.html",
          ctemplate::DO_NOT_STRIP);
		tpl->Expand(html, &dict);
	}
};
